...
class BiliShareAttach {
    ...
    public void login(Activity activity, SocializeMedia type,SocializeListeners.ShareListener listener){
        if (mShareConfiguration == null) {
            throw new IllegalArgumentException("BiliShareConfiguration must be initialized before share");
        }
        ...
        if (mCurrentHandler != null) {
            try {
                ...
                mInnerProxyListener.onStart(type);
                mCurrentHandler.login(mInnerProxyListener);
    ...
    private IHandler.InnerShareListener mInnerProxyListener = new IHandler.InnerShareListener() {
        ...
        public void onSuccess(SocializeMedia type, int code,Result result) {
            if (mOuterShareListener != null) {
                mOuterShareListener.onSuccess(type, code,result);
            }
            release(type);
        }
